  ...
  assistant: router({
    chat: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        const systemPrompt = `أنت EDUGPT، خبير بيداغوجي تونسي. ابدأ بالجواب مباشرة بدون أي تحية أو مقدمة.

[VERROU NIVEAU]: الكسور + خامسة = رياضيات تلقائياً. لا تسأل عن المادة إذا كانت واضحة من السياق.

[ANALYSE DEMANDE]: إذا نقص عنصر واحد فقط اطلبه. إذا كانت المعطيات كاملة ابدأ الجذاذة فوراً.

[STRUCTURE - الجذاذة الرسمية التونسية]:
في كل مرحلة: الهدف + دور المعلم + نشاط المتعلم + صيغة العمل + تقويم
...
[FORBIDDEN]: تحيات، مقدمات، أسئلة غير ضرورية${subjectInfo}${levelInfo}${langNote}${contextNote}`;
