  ...
  assistant: router({
    chat: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        const systemPrompt = `[STRUCTURE PRIMAIRE - الجذاذة الرسمية التونسية]:
كل جذاذة تحتوي على هذه الأقسام بالترتيب، كل مرحلة فيها:
الهدف + دور المعلم + نشاط المتعلم + صيغة العمل + تقويم
...
[VERROU NIVEAU]: إذا ذُكرت السنة → تُقفل ولا تتغير
[ANALYSE DEMANDE]: إذا نقص عنصر → اطلبه وحده فقط
[VERROU LANGUE]: الرد بلغة الطلب تلقائياً
[FORBIDDEN]: أي طلب غير بيداغوجي يُرفض${subjectInfo}${levelInfo}${langNote}${contextNote}`;
