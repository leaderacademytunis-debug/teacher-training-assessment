...
const DEFAULT_TOOLS = [
  ...
  { toolKey: "repartition_journaliere", nameAr: "التوزيع اليومي", nameFr: "Répartition Journalière", nameEn: "Daily Schedule", icon: "Calendar", category: "planning", freeAccess: true, freeLimitPerMonth: 5, proLimitPerMonth: 30, premiumLimitPerMonth: 0 },
  { toolKey: "reference_content_manager", nameAr: "إدارة المحتوى المرجعي", nameFr: "Gestion du contenu de référence", nameEn: "Reference Content Manager", icon: "Database", category: "planning", freeAccess: true, freeLimitPerMonth: 0, proLimitPerMonth: 0, premiumLimitPerMonth: 0 },
];
...
export const adminControlRouter = router({
  ...
  // ============================================
  // 6. NAME CORRECTION & CERTIFICATE REGENERATION
  // ============================================

  // Get user details with certificates for name correction
  getUserForNameCorrection: adminOnlyProcedure
    .input(z.object({ userId: z.number() }))
    .query(async ({ input }) => {
      ...
      const userCerts = await db.select({
        ...
        correctedName: certificates.correctedName,
        issuedAt: certificates.issuedAt,
        lastRegeneratedAt: certificates.lastRegeneratedAt,
      }).from(certificates).where(eq(certificates.userId, input.userId)).orderBy(desc(certificates.issuedAt));

      // Get course names for each certificate
      const certsWithCourses = await Promise.all(userCerts.map(async (cert) => {
        const [course] = await db.select({ titleAr: courses.titleAr, category: courses.category }).from(courses).where(eq(courses.id, cert.courseId));
        return { ...cert, courseTitleAr: course?.titleAr || "غير معروف", courseCategory: course?.category || "" };
      }));

      const editHistory = await db.select().from(nameEditHistory)
        .where(eq(nameEditHistory.userId, input.userId))
      ...
      return {
        ...
        certificates: certsWithCourses,
        editHistory,
      };
    }),

  // Search users for name correction
  searchUsersForCorrection: adminOnlyProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await db.select({
        ...
        firstNameFr: users.firstNameFr,
        lastNameFr: users.lastNameFr,
        email: users.email,
      }).from(users).where(
        or(
          like(users.name, `%${input.query}%`),
          like(users.firstNameAr, `%${input.query}%`),
          like(users.lastNameAr, `%${input.query}%`),
          like(users.firstNameFr, `%${input.query}%`),
          like(users.lastNameFr, `%${input.query}%`),
  ...
  // Correct participant name and optionally regenerate certificates
  correctParticipantName: adminOnlyProcedure
    .input(z.object({
      userId: z.number(),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Save edit history
      await db.insert(nameEditHistory).values({
        userId: input.userId,
        ...
        newFirstNameAr: input.firstNameAr || user.firstNameAr || null,
        newLastNameAr: input.lastNameAr || user.lastNameAr || null,
        newFirstNameFr: input.firstNameFr || user.firstNameFr || null,
        newLastNameFr: input.lastNameFr || user.lastNameFr || null,
      ...
      if (input.firstNameAr !== undefined) updateData.firstNameAr = input.firstNameAr;
      if (input.lastNameAr !== undefined) updateData.lastNameAr = input.lastNameAr;
      if (input.firstNameFr !== undefined) updateData.firstNameFr = input.firstNameFr;
      if (input.lastNameFr !== undefined) updateData.lastNameFr = input.lastNameFr;
      ...
      if (input.regenerateCertificates) {
        const userCerts = await db.select().from(certificates).where(eq(certificates.userId, input.userId));

        const newFirstNameAr = input.firstNameAr || user.firstNameAr || '';
        const newLastNameAr = input.lastNameAr || user.lastNameAr || '';
        ...
        const [lastEdit] = await db.select().from(nameEditHistory)
          .where(and(
            eq(nameEditHistory.userId, input.userId),
        ...
        if (lastEdit) {
          await db.update(nameEditHistory).set({
            certificatesRegenerated: regeneratedCount,
          }).where(eq(nameEditHistory.id, lastEdit.id));
        }
      }
      ...
      return {
        ...
        message: regeneratedCount > 0
          ? `تم تصحيح الاسم وإعادة إصدار ${regeneratedCount} شهادة بنجاح`
          : "تم تصحيح الاسم بنجاح",
      };
    }),

  // Get name edit history for all users (admin overview)
  getNameEditHistory: adminOnlyProcedure
    .input(z.object({ limit: z.number().default(50) }))
    .query(async ({ input }) => {
      const db = (await getDb())!;
      const history = await db.select({
        id: nameEditHistory.id,
        userId: nameEditHistory.userId,
        editedBy: nameEditHistory.editedBy,
        previousFirstNameAr: nameEditHistory.previousFirstNameAr,
        previousLastNameAr: nameEditHistory.previousLastNameAr,
        newFirstNameAr: nameEditHistory.newFirstNameAr,
        newLastNameAr: nameEditHistory.newLastNameAr,
        reason: nameEditHistory.reason,
        certificatesRegenerated: nameEditHistory.certificatesRegenerated,
        createdAt: nameEditHistory.createdAt,
      }).from(nameEditHistory)
        .orderBy(desc(nameEditHistory.createdAt))
        .limit(input.limit);

      // Enrich with user names
      const enriched = await Promise.all(history.map(async (h) => {
        const [participant] = await db.select({ name: users.name, email: users.email }).from(users).where(eq(users.id, h.userId));
        const [editor] = await db.select({ name: users.name }).from(users).where(eq(users.id, h.editedBy));
        return {
          ...h,
