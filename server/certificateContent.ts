...
export const CERTIFICATE_CONTENTS: Record<string, CertificateContent> = {
  ...
  'تأهيل مرافقي التلاميذ ذوي الصعوبات': {
    ...
    title: 'شهادة',
    subtitle: 'أُسندت هذه الشهادة إلى السيد/ة:',
    mainText: 'للمشاركة الفاعلة في دورة تكوينية بعنوان: "المرافق التربوي للتلاميذ ذوي اضطرابات وصعوبات التعلم"',
    axes: [
      'المصطلحات والمفاهيم المتصلة بصعوبات واضطرابات وبطء التعلم والتأخر الدراسي',
      'القدرات العامة والإنجاز الفعلي واضطرابات التعلم النمائية والأكاديمية',
      'اضطرابات فرط الحركة وقلة الانتباه',
      'عسر القراءة، عسر الكتابة، الحبسة، والعسر الحسابي',
      'رصد وتصنيف صعوبات التعلم والبحث عن مصادرها',
      'التدخلات البيداغوجية الملائمة وبناء جهاز علاجي مناسب لمرافقة ذوي صعوبات التعلم'
    ]
  },
...
export function getCertificateContent(courseTitle: string): CertificateContent | null {
  // Trim whitespace from course title
  const trimmed = courseTitle.trim();
  
  // Try exact match first
  if (CERTIFICATE_CONTENTS[trimmed]) {
    return CERTIFICATE_CONTENTS[trimmed];
  }
  
  // Try without diacritics
  const normalized = trimmed.normalize('NFD').replace(/[\u064B-\u065F]/g, '');
  
  for (const [key, content] of Object.entries(CERTIFICATE_CONTENTS)) {
    const normalizedKey = key.trim().normalize('NFD').replace(/[\u064B-\u065F]/g, '');
...
export function hasCertificateContent(courseTitle: string): boolean {
  return getCertificateContent(courseTitle) !== null;
}
