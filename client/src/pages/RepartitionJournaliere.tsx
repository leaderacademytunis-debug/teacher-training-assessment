...
export default function RepartitionJournaliere() {
  ...
  // Zustand: auto-fill from extracted_payload
  const { extracted_payload, clearPayload } = useExtractionStore();
  ...
  // Auto-fill from extracted_payload (from Textbook Library)
  useEffect(() => {
    if (extracted_payload) {
      setCommObjectifDetails(extracted_payload);
      toast.success("تم تعبئة المحتوى تلقائياً من المكتبة التفاعلية");
      clearPayload();
    }
  }, []);
